...
float4 PS(PSInput input) : COLOR0
{
  ...
  //input.Color;
  //float4 diffuseMap = tex2D(Texture1Sampler, input.TexCoord);
    float4 diffuseMap = tex2D(Texture1Sampler, input.TexCoord)*input.Color.x;
		diffuseMap += tex2D(Texture2Sampler, input.TexCoord)*input.Color.y;
		diffuseMap += tex2D(Texture3Sampler, input.TexCoord)*input.Color.z;
		diffuseMap += tex2D(Texture4Sampler, input.TexCoord)*input.Color.w;
...
#else
  return float4(DiffuseColor * diffuse, 1);
#endif
}
