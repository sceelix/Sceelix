...
{
    [MaterialTranslator(typeof(EmissiveMaterial))]
    public class EmissiveMeshMaterialTranslator : MeshMaterialTranslator<VertexPositionNormalTexture>
    {
        ...
        {
            EmissiveMaterial colorMaterial = (EmissiveMaterial) sceelixMaterial;
            ...
            Texture2D texture = TextureLoader.CreateColorTexture(_graphicsService.GraphicsDevice, colorMaterial.DefaultColor.ToXnaColor()); //ContentLoader.LoadTexture(graphicsService.GraphicsDevice, textureMaterial.Texture);

            var vector3Color = colorMaterial.DefaultColor.ToXnaColor().ToVector3();
            ...
            if (BuildPlatform.IsWindows)
            {
                ...
                EffectBinding materialEffectBinding = new EffectBinding(_graphicsService, _content.Load<Effect>("DigitalRune/Materials/MaterialEmissive"), null, EffectParameterHint.Material);
                materialEffectBinding.Set("DiffuseTexture", texture);
                //materialEffectBinding.Set("DiffuseColor", new Vector3(1, 1, 1));//  * 2f
                materialEffectBinding.Set("EmissiveColor", vector3Color);
